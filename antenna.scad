...
elementParameters = [
    for (param = calculateAntenna(2, frequencyToWavelength(869), 6, 6))
    [ round(param[0]), round(param[1]) ]
];
_ElemParamLength   = 0;
_ElemParamPosition = 1;

// TODO refactor reflector_position
boomLength = 300;
...
module allElements(reflector_position = 30, dipole_gap = 2, element_width = 10) {
    ...
    for (i = [0 : 1 : len(elementParameters) - 1]) {
        ...
        pos = reflector_position + param[_ElemParamPosition];
        if (isElementDriven(i)) {
            dipole(pos, param[_ElemParamLength], dipole_gap, width = element_width);
        }
        else {
            element(pos, param[_ElemParamLength], width = element_width);
        }
    }
}
...
module boomCutBox(
    ...
    positionsBetweenElements = [
        ...
        let(
            ...
            thisPos = thisParam[_ElemParamPosition] + element_width / 2,
            nextPos = nextParam[_ElemParamPosition] + element_width / 2,
